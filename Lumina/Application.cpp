...
std::string obtenirCheminAbsolu(const std::string& cheminRelatif) {
    try {
        std::filesystem::path chemin = std::filesystem::absolute(cheminRelatif);
        return chemin.string();
    } catch (const std::filesystem::filesystem_error& e) {
        std::cerr << "Erreur lors de la conversion du chemin : " << e.what() << std::endl;
        return {};
    }
}
...
void Application::InitializeWindow(int width, int height, bool customTitlebar) {
    ...
    glfwSetWindowPos(m_WindowHandle,
                      monitorX + (videoMode->width - g_MainWindowData.Width) / 2,
                      monitorY + (videoMode->height - g_MainWindowData.Height) / 2);

    // Rendre la fenêtre visible après l'avoir centrée
    glfwShowWindow(m_WindowHandle);
